"""Advanced Hybrid Search: Strict Pre-Filtering + ChromaDB Semantic Ranking.
...
Pipeline:
  ...
  2. Query ChromaDB for nearest-neighbor candidate embeddings.
  3. Cross-reference ChromaDB results against SQL-filtered candidates.
  4. Fallback to keyword relevance if no embeddings exist.
...
async def search_candidates(
    ...
    """Execute hybrid search: strict SQL pre-filter → ChromaDB semantic rank.
    ...
    qualifying candidates is done by ChromaDB cosine similarity (if available)
    or keyword relevance score.
    """
    strict_conditions = _build_strict_filters(intent, user_id=user_id)

    # ── Try ChromaDB semantic ranking first ──────────────────────────
    try:
        from backend.services.parsing.embedding import generate_embedding
        from backend.core.chromadb_client import query_similar_candidates

        query_embedding = generate_embedding(intent.semantic_query)

        # Query ChromaDB for nearest candidates (fetch more than limit for filtering)
        ...
        if chroma_results and chroma_results.get("ids") and chroma_results["ids"][0]:
            ...
            # Build a map: candidate_id → cosine distance
            dist_map = dict(zip(chroma_ids, chroma_distances))

            # Convert string IDs to UUIDs for SQL query
            candidate_uuids = []
            for cid in chroma_ids:
                try:
                    candidate_uuids.append(uuid.UUID(cid))
                except ValueError:
                    continue

            if candidate_uuids:
                # Apply strict SQL filters + restrict to ChromaDB results
                stmt = (
                    select(Candidate)
                    .where(and_(
                ...
                if rows:
                    # Sort by ChromaDB distance (ascending = more similar)
                    scored = []
                    for row in rows:
                        cid = str(row.id)
                        dist = dist_map.get(cid, 1.0)
                        similarity = max(0.0, min(1.0 - dist, 1.0))
                        scored.append(_row_to_dict(row, round(similarity, 4)))
