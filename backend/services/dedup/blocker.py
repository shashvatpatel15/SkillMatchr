...
4. Embedding block — ChromaDB nearest-neighbor (cosine similarity)
...
async def find_potential_matches(
    ...
    # 4. Embedding block — ChromaDB nearest neighbors (top 10)
    ...
    if embedding is not None:
        try:
            ...
            if chroma_results and chroma_results.get("ids") and chroma_results["ids"][0]:
                emb_ids = chroma_results["ids"][0]

                # Convert to UUIDs and fetch from DB
                emb_uuids = []
                for cid in emb_ids:
                    try:
                        uid = uuid.UUID(cid)
                ...
                if emb_uuids:
                    emb_query = (
                        select(Candidate)
                        .where(and_(
                            Candidate.id.in_(emb_uuids),
                            Candidate.ingestion_status.in_(_DEDUP_STATUSES),
                        ))
                    )
                    if user_id:
                        emb_query = emb_query.where(Candidate.created_by == uuid.UUID(user_id))

                    emb_result = await session.execute(emb_query)
                    emb_candidates = list(emb_result.scalars().all())

                    logger.info(
                        "BLOCKER: %d candidates from ChromaDB nearest-neighbor for '%s'",
            ...
            logger.warning("BLOCKER: ChromaDB embedding block failed: %s", e)
